...
func connect() (*amqp.Connection, error) {
	for {
		...
		if err != nil {
			log.Printf("Failed to connect to RabbitMQ: %v. Retrying in few seconds...", err)
...
func (d *Deployment) processMessages(ctx context.Context, msgs <-chan amqp.Delivery) {
	log.Printf(" [*] Waiting for messages from rabbitmq")
	for msg := range msgs {
		d.processEachMessage(ctx, msg)
	}
}
...
func (d *Deployment) ReceiveRabbitMq(ctx context.Context) {
	...
	d.InitRabbitMq(ctx)
	go d.checkConnection(ctx)

}

func (d *Deployment) checkConnection(ctx context.Context) {
	for {
		time.Sleep(time.Duration(SLEEP) * time.Second)
		if conn.IsClosed() {
			log.Println("RabbitMQ connection is closed, stopping receiving...")
...
func (d *Deployment) InitRabbitMq(ctx context.Context) {
	conn, err = connect()
	failOnError(err, "Failed to connect to RabbitMQ")
